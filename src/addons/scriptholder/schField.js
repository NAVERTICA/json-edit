...
(function (root, factory) {
    ...
    formatHints.array.schField = function (name, type, id, opts, required, priv, util, fromOneOf) {
        var
            options = opts["je:schField"] || {},
			codeId = id + (options.id || "")+"-schField",
    ...
    collectHints.array.schField = function (key, field, schema, priv) {
		var selected = $(field).find("select[loadtype='Field']").find('option:selected');
		var result = [];
		
		if(schema["je:schField"].followLookup == true){
