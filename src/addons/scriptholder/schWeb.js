...
(function (root, factory) {
    ...
    formatHints.string.schWeb = function (name, type, id, opts, required, priv, util) {
	if(typeof(ContextGuids)=="object" && ContextGuids.userlogin == "nvrpoint\\tpilich")debugger;
        var
            options = opts["je:schWeb"] || {},
			codeId = id + (options.id || "")+"-schWeb",
        ...
        util.events.rendered.handleOnce(function (selector) {
			schGlobal.loadWeb(codeId, content, selector, options);
        });
    ...
    collectHints.string.schWeb = function (key, field, schema, priv) {
